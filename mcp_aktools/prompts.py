...
def prompt_analyze_stock(symbol: str):
    ...
    1. 获取当前时间确认市场状态。
    2. 使用 `market_prices` 获取最近 30 天的日线数据，分析 MACD 和布林带走势。
...
def prompt_analyze_crypto(symbol: str):
    ...
    1. 使用 `crypto_prices` 获取最近 100 根 4H K线数据，分析 MACD、RSI 和布林带走势。
    2. 调用 `crypto_sentiment_metrics` 获取情绪指标，判断市场情绪偏向。
...
def prompt_crypto_pulse():
    ...
    4. 使用 `crypto_prices` 获取 BTC-USDT 的 1D K线，判断大盘趋势。
